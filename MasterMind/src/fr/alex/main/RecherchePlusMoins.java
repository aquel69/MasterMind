...
public class RecherchePlusMoins  {
	...
	private static Logger logger = LogManager.getLogger(Combinaison.class.getName());
		...
		//choix du mode développeur ou normal
	...
	public void init() {
		do {
			reponseModeDeveloppeur = verificationSaisieUtilisateurMultiple("Entrer 1 pour être en mode développeur ou 2 pour être en mode normal : ", "ne tapez pas de lettre, rentrez des chiffres", "veuillez rentrer au moins un chiffre");
		...
		do {
			...
			//switch contenant les Différents modes de jeux
			...
			do {
				...
				do {
					...
					do {
						...
						reponse = verificationSaisieUtilisateurMultiple("Veuillez entrer une combinaison à  "+ RessourcesMaster.nbDeChiffreCombinaison +  " chiffres", "ne tapez pas de lettre, rentrez des chiffres", "veuillez rentrer au moins un chiffre");

					}while (reponse.length() != RessourcesMaster.nbDeChiffreCombinaison);

					//la chaine de caractère entrée par l'utilisateur puis convertie en entier dans un tableau placée dans la variable nbrePropose
			...
			case 2:
				...
				do {
					...
					//remise à  zero des listes et du nombre de coups
					nbDeCoupsJoues = 0;
					...
					//Boucle permettant de s'assurer du nombre de caractère saisie					
					do {
						reponse = verificationSaisieUtilisateurMultiple("Entrer la combinaison à  trouver par l'ordinateur", "ne tapez pas de lettre, rentrez des chiffres", "veuillez rentrer au moins un chiffre");
					}while (reponse.length() != RessourcesMaster.nbDeChiffreCombinaison);
			...
			case 3 :
				//Boucle permettant de recommencer une partie
				do {
					//remise à zéro des listes et du nb de coup joué pour une nouvelle partie
					nbDeCoupsJoues = 0;
					listeSauvegardeCombinaisonOrdi.clear();
					listeDesChiffresProposeParOrdinateur.clear();
					listeDesChiffresEntreParUtilisateur.clear();
					...
					else JOptionPane.showMessageDialog(null, "nb de coup dépassé pour les deux");
	 ...
	 * @param p1Demande
	 * @param p2Reponse
	 * @return le nombre entrÃ© par l'utilisateur et vérifie si les caractères entrés sont des chiffres ou des lettres ou si il n'y a rien
	 */
	public static String verificationSaisieUtilisateurMultiple(String p1Demande,String p2Reponse, String p3Reponse) {
		...
		do{
			...
			//verifie si aucun caractère n'est rentré
	 ...
	 * 
	 * @param pReponse2
	 * @return un booleen True si une lettre est rentrée par l'utilisateur
	 */
	 ...
	 * @param pReponse
	 * indique le signe sur chaque chiffre correspondant à  la diffèrence entre les deux tableaux et indiquant "+","-" ou "=" suivant le résultat
	 * @return 
	 */
	 ...
	 * @param pRandom
	 * @return la liste des chiffres proposés par l'ordinateur. A chaque tour de boucle, la fonction compare les chiffres des diffèrentes listes et propose une nouvelle combinaison
	 */
	public static ArrayList<Integer> remplissageDeLaListeDesChiffresProposeParLOrdinateur( int pNombreDeChiffreCombinaison, int pNbDeCoupsJoues,  ArrayList<Integer> pListeDesChiffresProposeParOrdinateur, ArrayList<Integer> pListeDesPrecedentsChiffreProposeParOrdinateur, ArrayList<Integer> pListeDesChiffresEntreParUtilisateur, ArrayList<Integer> pListeSauvegardeCombinaisonOrdi, Random pRandom){
		try {
				...
				for(int i = 0; i < pNombreDeChiffreCombinaison; i++) {	
					...
					int j = i;
						...
						if (pListeDesPrecedentsChiffreProposeParOrdinateur.get(i) > pListeDesChiffresEntreParUtilisateur.get(i)) {
							//boucle permettant que tant que le chiffre aléatoire choisi n'est pas différent que ceux déja proposé; recommence du début et parcour la liste complète en ajoutant avec 'j' le nombre correspondant Ã  touts les chiffres dÃ©ja jouÃ©s.
							//On sort lorsque le tableau est completement parcouru et qu'aucun chiffre sauvegardé n'est similaire au proposé
							if (pListeDesPrecedentsChiffreProposeParOrdinateur.get(i) <= 5 ) {
								...
								do{
									//comparaison du chiffre aléatoire proposÃ© et de la liste sauvegardée
									if (pListeDesChiffresProposeParOrdinateur.get(i) == pListeSauvegardeCombinaisonOrdi.get(j)/**|| listeDesChiffresProposeParOrdinateur.get(i) > nombreMaximum*/) {
										...
										//nouveau chiffre aléatoire et pour recommencer du début et reprendre Ã  l'index i	
										pListeDesChiffresProposeParOrdinateur.set(i, pRandom.nextInt(pListeDesPrecedentsChiffreProposeParOrdinateur.get(i)));
			...
			logger.debug("problÃ¨me sur la liste des chiffres proposÃ©s par l'ordinateur", e);
		}
	 ...
	 * @param pListeSauvegardeCombinaisonOrdi
	 * @param pListeDesChiffresProposeParOrdinateur
	 * @return la liste des prÃ©cedent chiffres jouÃ© pour ensuite que l'ordinateur ne les rejoue pas
	 */
	 ...
	 * @return un boolean qui indique si l'utilisateur a rÃ©ussi dans le mode 3
	 */
	 ...
	 * @param pNbreRecherche
	 * @param pNbDeCoups
	 * affichage du résultat trouvé et du nombre de coup qu'il a fallu Ã  l'utilisateur pour y arriver
	 */
	...
	public static void resultatFinalPerdantOrdi(ArrayList<Integer> pNbreProposeParUtilisateur, int pNbCoupMax) {
		...
		"Dans le nombre de coup imparti qui Ã©tait de " + pNbCoupMax +
